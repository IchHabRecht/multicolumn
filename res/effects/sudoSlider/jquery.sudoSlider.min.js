	...
	$.fn.sudoSlider = function(options) {
		...
		return this.each(function() {
			...
			function initSudoSlider(obj, destroyT, ajaxcallback) {
				...
				if (!dontCountinueInit) {
					...
					if (option[11]) {
						...
						if (option[6]) {
							for (i = option[40]; i >= 1; i--) {
								var tmpchild = li.eq(-option[40] + i - 1).clone();
								a = a + tmpchild.outerWidth(truev);
								tmpchild.css("margin-top", "-" + a + "px")
								ul.prepend(tmpchild);
								ul.append(li.eq(option[40] - i).clone().css({'z-index': '0'}));
							}
						}
						if (!option[6]) {
							for (i = option[40]; i >= 1; i--) {
					...
					if (option[0]) {
						...
						if (option[13]) {
							numericContainer = controls.prepend('<ol ' + option[14] + '></ol>').children();
							var substract = option[11] ? 1 : option[40];
							for (var i = 0; i < s - substract + 1; i++) {
								numericControls[i] = $(document.createElement("li")).attr({'rel': (i + 1)}).html('<a href="#"><span>' + option[15][i] + '</span></a>').appendTo(numericContainer).click(function() {
						...
						if (option[4]) {
							firstbutton = makecontrol(option[36], "first");
						}
						if (option[5]) {
					...
					option[40] = parseInt(option[40], 10)
					if (option[9]) {
						timeout = startAuto(option[10]);
					}
					if (option[20]) {
						$(option[20]).live('click', function() {
							a = $(this).attr('rel');
							if (a) {
								if (a == 'stop') {
									option[9] = falsev;
									...
									if (!option[11]) {
										baseSlider.setOption('continuous', truev);
									}
									timeout = startAuto(option[10]);
									option[9] = truev;
								}
					...
					if (destroyT) {
						...
						$.address.init(function(e) {
							if (!destroyT) {
								animate(filterUrlHash(e.value), falsev, falsev, falsev);
							}
						}).change(function(e) {
			...
			function imageCheck(file) {
				...
				if (ext == '.jpg' || ext == '.png' || ext == '.bmp' || ext == '.gif' || file.substr(len - 5, 5) == '.jpeg') {
					return truev;
				}
				return falsev;
			}
			...
			function fadeControl(fadeOpacity, fadetime, nextcontrol) {
				if (!option[11]) {
					...
					if (option[20]) {
						$(option[20]).filter(function(index) {
							return ($(this).attr("rel") == directionA || $(this).attr("rel") == directionB);
						}).fadeTo(fadetime, fadeOpacity, function() {
							if (fadeOpacity == 0) {
								$(this).hide();
							}
						});
			...
			function setCurrentElement(element, i) {
				$(element).filter(".current").removeClass("current").each(function() {
					if ($.isFunction(option[31])) {
						option[31].call(this, $(this).attr("rel"));
					}
				});
			...
			function setWidth(nwidth, speed) {
				obj.animate({width: nwidth}, {queue: falsev, duration: speed, easing: option[8]});
			};
			function setHeight(nheight, speed) {
				obj.animate({height: nheight}, {queue: falsev, duration: speed, easing: option[8]});
			};
			...
			function getSlidePos(slide, width) {
				var p = 0;
				var goTo = slide - (option[11] ? 1 - option[40] : 1);
				var add = option[11] ? add = option[40] : 0;
			...
			function adjust() {
				...
				if (!fading) {
					...
					if (option[11] && !option[21]) {
						if (t < option[40]) {
							afterAniCall(liConti.eq((t < 0) ? t + option[40] : t - option[40]), a);
						}
						if (t > ts - option[40]) {
							afterAniCall(liConti.eq(option[40] + t - ts - 1), a);
						}
					}
				}
			};
			...
			function filterDir(dir, ot) {
				...
				switch (dir) {
					case"next":
						nt = (ot >= ts) ? (option[11] ? nt + 1 : ts) : nt + 1;
						break;
					case"prev":
			...
			function ajaxLoad(i, l, adjust, speed, ajaxCallBack) {
				...
				var ajaxspeed = (fading) ? (!option[22] ? parseInt(option[23] * (2 / 5)) : option[23]) : speed;
				var tt = l + 1;
				if (imageCheck(option[25][l])) {
					targetslide.html(' ').append($(new Image()).attr('src', option[25][l]));
					runOnImagesLoaded(targetslide, function(img) {
						...
						function finishImageLoading(target, a) {
							a++;
							var width = target.width(), height = target.height(), slidewidth = target.parent().width();
							if (width == 0 && a != 20) {
								setTimeout(function() {
									finishImageLoading(target, a);
								}, 1);
								...
								if (width > slidewidth) {
									...
									}, 0).parent().animate({height: (height / width) * slidewidth}, 0).css('height', 'auto');
								}
								if ($.isFunction(option[28])) {
									option[28].call($(img), tt, truev);
								}
					...
					targetslide.load(option[25][l], function(response, status, xhr) {
						ajaxAdjust($(this), ajaxCallBack);
						if (adjust) {
							adjustPosition();
						}
						if (status == "error" || !$(this).html()) {
							$(this).html("Sorry but there was an error: " + (xhr.status ? xhr.status : 'no content') + " " + xhr.statusText);
						}
						if (status != "error" && $.isFunction(option[28])) {
			...
			function fadeto(i, clicked) {
				if (i != t && !destroyed && clickable) {
					...
					clickable = !clicked;
					var speed = (!clicked && !option[9] && option[17]) ? option[23] * (option[18] / option[7]) : option[23];
					...
					if (option[25] && option[25][ll]) {
						...
						if (option[22]) {
							target.clone().prependTo(obj).css({
								'z-index': '100000',
								'position': 'absolute',
								'list-style': 'none',
								'top': '0',
								'left': '0'
							}).hide().fadeIn(option[23], function() {
								if (fontsmoothing) {
									this.style.removeAttribute("filter");
								}
								clickable = truev;
								fading = truev;
								animate(ll, falsev, falsev, falsev);
								$(this).remove();
								if (option[17] && clicked) {
									window.location.hash = option[15][t];
								}
								fading = falsev;
			...
			function animate(dir, clicked, time, ajaxcallback) {
				if ((clickable && !destroyed && (filterDir(dir, ot) != t || init)) || ajaxcallback) {
					ajaxloading = falsev;
					clickable = (!clicked && !option[9]) ? truev : option[16];
					...
					var diff = Math.sqrt(Math.abs(ot - t)), speed = parseInt(diff * option[7]);
					if (!clicked && !option[9]) {
					...
					if (!dontCountinue) {
						if (t < 1 - 1) {
							var contiElement = liConti.eq(option[40] + t);
						}
						if (t > ts) {
							var contiElement = liConti.eq(t - s - option[40]);
						}
						if (!fading) {
							beforeAniCall(li.eq(i), i + 1)
							if (option[11]) {
								if (t < option[40]) {
									beforeAniCall(liConti.eq((t < 0) ? t + option[40] : t - option[40]), i + 1);
								}
								if (t > ts - option[40] || t == -option[40]) {
									beforeAniCall(liConti.eq((t == -option[40]) ? -1 : option[40] + t - ts - 1), i + 1);
								}
							}
						}
						...
						if (option[6]) {
							...
							ul.animate({marginTop: p}, {queue: falsev, duration: speed, easing: option[8], complete: adjust});
						}
						else {
							var p = getSlidePos(t, truev);
							ul.animate({marginLeft: p}, {queue: falsev, duration: speed, easing: option[8], complete: adjust});
						}
						;
						if (option[2]) {
							var fadetime = option[1];
							if (!clicked && !option[9]) {
								fadetime = (option[18] / option[7]) * option[1];
							}
							if (!time) {
								fadetime = 0;
							}
							if (fading) {
								fadetime = parseInt((option[23]) * (3 / 5));
							}
			...
			function returnOptionNumber(name) {
				var optionsUserName = ['controlsShow', 'controlsFadeSpeed', 'controlsFade', 'insertAfter', 'firstShow', 'lastShow', 'vertical', 'speed', 'ease', 'auto', 'pause', 'continuous', 'prevNext', 'numeric', 'numericAttr', 'numericText', 'clickableAni', 'history', 'speedhistory', 'autoheight', 'customLink', 'fade', 'crossFade', 'fadespeed', 'updateBefore', 'ajax', 'preloadAjax', 'startSlide', 'ajaxLoadFunction', 'beforeAniFunc', 'afterAniFunc', 'uncurrentFunc', 'currentFunc', 'prevHtml', 'nextHtml', 'loadingText', 'firstHtml', 'controlsAttr', 'lastHtml', 'autowidth', 'slideCount', 'resumePause'];
			...
			baseSlider.insertSlide = function(html, pos, numtext) {
				...
				option[15].splice(pos, 0, numtext || parseInt(pos, 10) + 1);
				baseSlider.init();
			}
